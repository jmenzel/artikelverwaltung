...
BEGIN_EVENT_TABLE( LieferantAdd, wxFrame)
	EVT_TEXT(ID_TFIELD_NAME, LieferantAdd::EventTextChangedName)
    EVT_BUTTON(ID_BUTTON_SAVE, LieferantAdd::EventOnClickButtonSave)
END_EVENT_TABLE()

LieferantAdd::LieferantAdd(MainWindow *parent)
    : wxFrame ( (wxFrame *) parent, -1, wxT("Lieferanten anlegen"), wxPoint(-1, -1), wxSize(420, 255))
{
    main = parent;
    parent_check = false;
    Initialize();
...
LieferantAdd::LieferantAdd(MainWindow *parent, CallbackBase &caller_update)
    : wxFrame ( (wxFrame *) parent, -1, wxT("Lieferanten anlegen"), wxPoint(-1, -1), wxSize(420, 255))
{
    main = parent;
...
void LieferantAdd::EventTextChangedName(wxCommandEvent &event)
{
...
void LieferantAdd::EventOnClickButtonSave(wxCommandEvent &event)
{
    if(tfield_name->GetValue().Length() > 0)
    {
        ...
        int id = db->addNewLieferant(tfield_name->GetValue(),
