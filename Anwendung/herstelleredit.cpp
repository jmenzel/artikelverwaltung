...
BEGIN_EVENT_TABLE( HerstellerEdit, wxFrame)
	EVT_TEXT(ID_TFIELD_NAME, HerstellerEdit::EventTextChangedName)
    EVT_BUTTON(ID_BUTTON_SAVE, HerstellerEdit::EventOnClickButtonSave)
END_EVENT_TABLE()

HerstellerEdit::HerstellerEdit(MainWindow *parent, unsigned int herstellerId)
    : wxFrame ( (wxFrame *) parent, -1, wxT("Hersteller anlegen"), wxPoint(-1, -1), wxSize(420, 255))
{
	...
	db->getHerstellerById(*new Callback<HerstellerEdit>(this, &HerstellerEdit::FillHerstellerData), this->hId);
	
	this->Show();
}

HerstellerEdit::~HerstellerEdit()
{
}

void HerstellerEdit::FillHerstellerData(vector< vector< wxString > > data)
{
	if(data.size() > 0)
	{
...
void HerstellerEdit::EventTextChangedName(wxCommandEvent &event)
{
...
void HerstellerEdit::EventOnClickButtonSave(wxCommandEvent &event)
{
    if(tfield_name->GetValue().Length() > 0)
    {
        ...
        db->updateHerstellerById(this->hId,
